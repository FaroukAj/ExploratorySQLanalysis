...
SELECT SEX, COUNT(SEX) AS gender_distribution
FROM claims_data
GROUP BY SEX;

/* OR USING PARTITION BY */

SELECT DISTINCT SEX, COUNT(SEX) OVER (PARTITION BY SEX) AS gender_distribution
FROM claims_data;
...
SELECT company, SUM(TOTAL_PRICE) AS Total_amount
FROM claims_data
GROUP BY company;


-- Calculate the total claimed amount for each service type.
SELECT service_type, SUM(TOTAL_PRICE) AS Total_amount
FROM claims_data
GROUP BY service_type;

-- Identify the top 5 providers with the highest total claimed amount.
SELECT Provider_name, SUM(TOTAL_PRICE) AS Total_amount
FROM claims_data
