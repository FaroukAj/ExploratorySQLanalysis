...
SELECT EmployeeID, recorddate_key, ROW_NUMBER() OVER (PARTITION BY EmployeeID ORDER BY recorddate_key DESC) AS record_date_count
FROM Employeedata
)
...
SELECT department_name, ROUND(AVG(age)) AS AVG_Age
FROM employeedata
GROUP BY department_name;

-- Calculate the cumulative length of service for each employee, ordered by their hire date.
SELECT employeeID, SUM(length_of_service) AS cumulative_length_of_service
FROM employeedata
GROUP BY employeeID;

-- Determine the top 5 cities with the highest number of employees.
SELECT city_name, COUNT(EmployeeID) AS Employee_count
FROM employeedata
GROUP BY city_name
...
SELECT department_name, total_count, ROUND((male_count/total_count) * 100.00, 2) AS male_percentage, 
ROUND((female_count/total_count) * 100.00, 2) AS female_percentage
FROM employee_percentage;
								/* OR USING PARTITION BY */ 
SELECT department_name, gender_full,
...
SELECT job_title, COUNT(EmployeeID) AS'employees_laid-off'
FROM employeedata
...
SELECT store_name, ROUND(AVG(age))
FROM employeedata
WHERE status = "ACTIVE"
GROUP BY store_name;
